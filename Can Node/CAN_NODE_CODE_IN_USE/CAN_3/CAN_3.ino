...
unsigned char IMU1[8] = { 
  0, 0, 0, 0, 0, 0, 0, 0};
  unsigned char VoltageId=0x32;
unsigned char IMUId1 = 0x2B;
...
int p;
float voltage;
unsigned char Pressure1Id = 0x29;
unsigned char Pressure2Id = 0x2A;
...
void setup() 
{
  ...
  pinMode(A3, INPUT);
  pinMode(A0, INPUT);
  if(CAN.begin(CAN_500KBPS) ==CAN_OK) Serial.println("CAN bus initilized sucessfully at 500KBPS");
  else Serial.println("CAN bus initilzation failed");
...
mpu_initalise:
Serial.println("Try to initalise IMU");
  mpu.initialize();
...
void loop() {
  ...
  total_pressure0+=(analogRead(A2)-514.09)/1.2172;
  total_pressure1+=((analogRead(A3)-516.95)/1.2164);
  total_voltage+=analogRead(A0)/41.495;
  countp++;
  ...
  if(millis()-pressure_timer>500)//every 0.5 sec
  {
    ...
    voltage=total_voltage/countp;
    Voltage[7]=0;
    if(voltage<-1) Voltage[7]=2;
    if(voltage<-3) Voltage[7]=1;
    Voltage[0]=int(voltage*10);
    CAN.sendMsgBuf(VoltageId, 0, 8, Voltage);
#ifdef PRINT_PRESSURE
  ...
  if (dmpReady) 
  {
    ...
    else if (mpuIntStatus & 0x02) 
    {
      ...
      yaw=IMUAngAdjust(yaw, 265, false);
